...
```bash
# База (Supabase облако) — session mode 5432
DATABASE_URL_BOT=postgresql+asyncpg://postgres.vgwwidekswqeffltqhag:${PASS}@aws-1-eu-central-1.pooler.supabase.com:5432/postgres?search_path=mesh_bot
...
```bash
# База (Supabase облако) — transaction mode 6543!
DATABASE_URL=postgresql://postgres.vgwwidekswqeffltqhag:${PASS}@aws-1-eu-central-1.pooler.supabase.com:6543/postgres?schema=mesh_cp&pgbouncer=true
...
| Где | Что хранится |
|---|---|
| `~/.ai/env/keys.env` | `SUPABASE_DB_PASSWORD`, `SUPABASE_MANAGEMENT_PAT`, `DOCKER_TOKEN`, `GH_PAT` |
...
```bash
# 1. Новый пароль в ~/.ai/env/keys.env → SUPABASE_DB_PASSWORD
# 2. Сбросить в Supabase:
curl -X PATCH "https://api.supabase.com/v1/projects/vgwwidekswqeffltqhag/database/password" \
  -H "Authorization: Bearer ${SUPABASE_MANAGEMENT_PAT}" \
  -H "Content-Type: application/json" \
  -d '{"password":"${NEW_PASS}"}'
# 3. Обновить в frontend/.env, backend/cp.env
