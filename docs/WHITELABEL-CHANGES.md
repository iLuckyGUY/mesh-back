...
> Последний merge: v3.58.0 (2026-06-02)
...
Задаются через `bot.env` / `stack.env` — переопределяют дефолты.
Добавлены в `Settings` класс как pydantic fields → читаются через `settings.*` геттеры.

---

### 3. Подключение полей config к коду (было захардкожено)

> ⚠️ После merge v3.58.0 upstream восстановил hardcoded строки.
> Применено снова в коммите `5343f380`. При следующем merge проверять эти же файлы.
